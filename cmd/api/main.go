...
func main() {
	...
	questionRepo := postgres.NewQuizQuestionRepository(db)
	...
	questionUsecase := usecase.NewQuizQuestionUsecase(questionRepo)
	quizUsecase := usecase.NewQuizUsecase(quizRepo, attemptRepo, questionRepo)
	examUsecase := usecase.NewExamUsecase(examRepo, examQuestionRepo, examAttemptRepo)
	examQuestionUsecase := usecase.NewExamQuestionUsecase(examQuestionRepo)
	...
	protedcted.Use(middleware.RequireAuth())
	{
		...
		http.NewQuizQuestionHandler(r, questionUsecase, enrollmentRepo)
